...
/** Exact-name SYSTEM allowances: daemon-launch environment only. The startup
 *  provider bootstrap (claude-provider-env.ts) must NEVER extract these from
 *  user settings — PATH/HOME/locale/CLAUDE_CONFIG_DIR belong to the daemon's
 *  own launch context (plan §4.1 rule 6). */
const SYSTEM_EXACT_ALLOW: ReadonlySet<string> = new Set(["PATH", "HOME", "LANG", "TMPDIR", "CLAUDE_CONFIG_DIR"]);

/** Exact-name PROVIDER/TLS allowances: the only non-prefixed keys the startup
 *  provider bootstrap may fill from the user-level Claude settings `env`. */
const PROVIDER_EXACT_ALLOW: ReadonlySet<string> = new Set([
  "SSL_CERT_FILE",
  "SSL_CERT_DIR",
  "NODE_EXTRA_CA_CERTS",
...
/** The SINGLE provider-field classification (review STD-3 extended to the
 *  startup provider bootstrap): every key the bootstrap may extract from the
 *  user-level settings `env` AND every provider/TLS/proxy key buildAgentEnv
 *  forwards. System keys stay allow-listed for forwarding but are never
 *  settings-extractable, so `isAllowed` = system ∪ locale ∪ provider. */
export function isProviderEnvKey(key: string): boolean {
  return PROVIDER_EXACT_ALLOW.has(key) || PROXY_NAMES.has(key) || key.startsWith("ANTHROPIC_");
}
...
function isAllowed(key: string): boolean {
  return SYSTEM_EXACT_ALLOW.has(key) || key.startsWith("LC_") || isProviderEnvKey(key);
}
