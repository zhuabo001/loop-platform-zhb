...
2. **workdir jail 是 cwd 选择机制，不是运行时文件系统安全边界**。模块文档与测试注释均明示：阻止运行中进程越界读写是批次三 OS sandbox 的职责（`failIfUnavailable: true`，禁止 unsandboxed fallback）。本批只承诺 **resolve 返回时刻的时点正确性**：选定 cwd 在 resolve 时落在 daemon roots ∩ server roots 内，或落在隔离的 per-run scratch。Node spawn 的 cwd 是字符串路径，resolve→spawn 之间存在 TOCTOU 窗口（句柄级原子化不可用）；批次三 adapter 必须在 spawn 前重新校验 cwd，并以 fail-closed OS sandbox 作为唯一真正的运行时边界。
...
4. **per-run scratch 永不复用**：`sha256(loopId+runId)` 只做 mkdtemp 前缀，随机后缀保证同一 runId 重入也得新目录（J21）；权限钉死 `0700`（J22）。`release()` fail-closed：只删本 jail 铸造、仍是 per-jail scratch 根目录直接子级、未被替换为符号链接的目录——lstat 识别符号链接替换，校验或删除失败一律抛错且什么都不删（J23–J25）。
...
6. **一个 spawn 一个进程组，终止语义收敛为一条幂等路径**：`detached: true` 使 child 成为组长；timeout、AbortSignal、consumer 抛错三类触发器**先到者决定 completion kind**（S8）；TERM → 5s grace → KILL 对整组生效；直接 child `close` 后再查一次进程组，残留孙进程走同一路径（S9）；返回前 close 已触发、stdio 已排空、进程组不存在（S17）。`kill(-pgid, 0)` 仅是存活探测：`ESRCH` 视为已结束（S16），瞬时 `EPERM` 保守视为“可能仍存活”并继续有界收口（S21）；真正发送 TERM/KILL 的非 `ESRCH` 错误仍立即 reject 并脱离该 child（清计时器/监听、销毁 stdio、unref），绝不等待一个可能永远无法促成的 `close`，绝不静默误报活性（S19）。
7. **stdio 每流 1 MiB 上限，头尾各半**：未超 cap 时 head+tail 恰好是完整流（无损），第一次丢字节即翻 truncated 标志（S11–S12）。捕获器在 `push` 时对输入字节取快照，consumer 收到独立副本，任一方事后修改 Buffer 都不能篡改已捕获输出（CS4/S20）。`onStdout/onStderr` chunk 回调按到达顺序先于捕获触发——批次三的 stream-json 解析器直接复用，不需要改本模块；consumer 抛错记 `consumer-error` 并终止整组（S14）。
...
- 2026-08-19（Round 2 代码审查修复，复审结论「不通过」后的返工）：
  1. **CappedStream 重写为字节累积 + 结果时一次解码**（审查 P1）：Round 1 的「切口在当前 chunk 内回退」没有跨 chunk 的 pending-byte 状态——pipe 分块任意（65536 % 3 ≠ 0），head 可冻结在跨 chunk 的残缺字符上（S18 定向 5 跑 4 败、确定性复现重编码 1,048,578 字节超 cap）。新实现：未超 cap 保留全部字节（恰好 1 MiB 不误标 truncated，CS2）；超 cap 冻结 head=前 512 KiB、滚动 tail=后 512 KiB（内存有界）；`text()` 时才在原始字节上对齐切口（head 丢弃尾部残缺序列 ≤3 字节、tail 跳过前导 continuation ≤3 字节）。分块边界与正确性结构性解耦，S18 转为稳定（修复后定向 5/5）。新增字节级确定性用例 CS1–CS3。
  2. **终止失败立即收口**（审查 P1）：非 ESRCH kill 错误（EPERM）此前捕获后要等 child `close` 才 reject——杀不动的长驻 child 令调用方永久 pending（审查复现：200ms 仍 pending，~734ms 随 child 自然退出才 reject）。现 `terminate()` 捕获错误后经 `failFatally` **立即 reject 并脱离该 child**（清计时器/监听、销毁 stdio、unref）；各收口路径由单一 `settled` 标志去重。新增测试专用 seam `SpawnOptions.killImpl`（与 `graceMs` 同性，生产不得设置）支撑确定性 EPERM 用例 S19——POSIX 无可便携的 EPERM 注入手段，seam 注入是确定性验证的唯一途径；may-be-probs 第 1 项随之重新关闭。
  3. **S8 确定性半例补齐**（审查 P2）：新增 F3——timeout（80ms）显著早于 abort（400ms）时 kind 恒为 `timed-out`。pin-only 提交（实现经检查本就满足，缺口在证据而非行为），沿用 ADR-004 决策 10 的例外注明惯例。
  4. **TOCTOU 口径一致化**（审查 P2）：主决策 2 改写为与修订记录第 3 条相同的时点契约表述（resolve 时刻正确性 + 批次三 spawn 前重校验义务），消除主决策与修订记录互相矛盾。
...
- 2026-09-02（Issue #38 修复，计划 `docs/plan/codex-fix-claude-runner-plan.md`）：
  1. **启动期受控 provider bootstrap**：`--setting-sources ""` 同时禁用了 Claude 加载用户级 `settings.json`——其中的 `env` 携带本机实际可用的 provider endpoint、认证 token 与模型映射，被隔离的调用路径因此报 `Not logged in`（并非本机未登录）。修复为启动期深模块 `resolveClaudeProviderEnv`：只读 `<CLAUDE_CONFIG_DIR|HOME/.claude>/settings.json` 一个文件的顶层 `env`，只提取与 agent env allow-list 对齐的 provider/TLS/proxy 字段，显式启动环境优先、settings 仅补缺；缺文件不算错误，文件不可读/非 JSON object/`env` 非 object/允许字段非 string 一律启动 fail-closed，错误不含文件内容与字段值。**已否决**：`--setting-sources user`（任意 `env` 会绕过 daemon 白名单与 secretValues 收集）、credential 写入 `--settings` argv（进程参数可观测）、扩大 sandbox 文件白名单（无必要且扩大 Agent 文件能力）。
  2. **统一 provider 字段分类**：`agent-env.ts` 导出单一 `isProviderEnvKey` 分类函数，供 `buildAgentEnv()` 与 bootstrap 复用，两处不得复制 allow-list；PATH/HOME/locale/`CLAUDE_CONFIG_DIR` 保持「可转发但永不可从 settings 提取」。bootstrap 产物经既有 `buildAgentEnv` → `collectSecretValues` → `redactSecrets` 管道，settings 来源的 credential 由此复用全部既有脱敏规则，不开第二条 secret 通道；`agent-env.test.ts` 以 drift detector 钉死分类一致性。
