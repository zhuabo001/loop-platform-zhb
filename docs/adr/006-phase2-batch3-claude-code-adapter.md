...
- 2026-08-24：Batch 4 第三轮整改——E2E 改为消费 daemon **生产 probe 实际钉住**的 realpath/version/sha256，并要求操作者显式批准 hash；测试端删除 shell 解析。日志改为全生命周期、跨 chunk、sticky credential 检测，诊断仅保留 64 KiB 脱敏 tail；daemon 以独立进程组启动，supervisor 等待 ChildProcess `close` 并收口已观察到的 detached 后代进程组。以上行为由不调用真实 Claude 的确定性测试覆盖，等待后续三轨复审和人工 smoke/E2E 复跑后再关闭 Issue #10、恢复 Phase 2 完成状态。实现历史偏差同时记账：`0104cee` 同时包含 red 测试与生产改动，不作为独立 red commit 证据，也不改写共享 Git 历史。
