...
- **Batch 1 — 时间语义与持久化基础（Day 1–3）：Round 3 复审中**（2026-08-25，ADR-007）
  ...
  - 测试覆盖：M 组（迁移与 schema，6 tests）、D 组（cron/timezone/DST，7 tests）、C 组（配置状态机，8 tests）——21 tests；M1/M5 使用文件型 PGlite、Drizzle journal/runner 和关闭重开路径。
  ...
  - 复审：Round 1、Round 2 均不通过；Round 2 修复提交 `67cb80a` 已覆盖通用 DST gap、标准 weekday 7、显式 five-part mode、真实文件库迁移与事务故障回滚，等待 Round 3 三轨复审核销。
  - 开放核销项：[Issue #19](https://github.com/zhuabo001/loop-platform-zhb/issues/19)、[#20](https://github.com/zhuabo001/loop-platform-zhb/issues/20)、[#21](https://github.com/zhuabo001/loop-platform-zhb/issues/21)、[#22](https://github.com/zhuabo001/loop-platform-zhb/issues/22)。
