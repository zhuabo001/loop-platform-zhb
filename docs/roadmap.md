...
- **Batch 1 — 时间语义与持久化基础（Day 1–3）：已完成**（2026-08-25，ADR-007）
  ...
  - 测试覆盖：M 组（迁移与 schema，6 tests）、D 组（cron/timezone/DST，6 tests）、C 组（配置状态机，8 tests）——20 tests 全绿；M1 真实旧库升级路径（0000-0001 → 0002）验证字段保留与默认值。
  ...
  - 复审：Round 1 不通过（DST gap 语义错误、M1 未覆盖真实旧库、manual-only timezone 校验绕过、文档收口未完成）；Round 2 全部 P1 修复并通过质量门（263 tests passed、typecheck、build、whitespace check），验收记录见 `docs/handoff/codex-handoff-phase3-batch1-round2.md`。
  - **Batch 1 已最终收口**（2026-08-25）。
