...
2026-09-08 复审补充：A–D 使用诊断账本；最终生产 P 与拒绝 R 使用独立验收账本，最多 4 次、90 分钟，并且必须由操作者显式设置 `LOOPZHB_COMPAT_ACCEPTANCE_BUDGET_USD` 正数目标。账本固定在用户持久目录，不随 `--evidence-dir` 改变；两个账本共享 `.spend-ledgers.lock`，检查与 `started` 预留受同一进程间排他锁保护，完成时按 `runId` 更新最新账本。任一账本出现 started 未完成、费用未知或格式损坏，所有变体均 fail-closed；次数/时间/金额限制仍按各自阶段预算计算。切换变体、设置新验收预算或更换 evidence 目录均不能代替既有未知费用的核销。锁只在读取、预留和完成更新期间持有；调用运行期间由持久 started 记录阻止并发调用。setup、预留失败及调用退出均须回收临时能力，账本写失败也不得留下临时写文件。
...
1. 普通 Bash 检查包括成功命令和故意失败命令；分别核对工具结果与文件证据，不能只检查 Claude 最终退出码。
   cwd/OpenSSL marker 检测使用完整 tool-result 内容；限长仅用于证据展示，不能用于错误检测，且 tool-use 命令文本不作为错误来源。
