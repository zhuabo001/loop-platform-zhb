...
3. 先对所有有效 Loop 调用现有 `reconcile()` 注册 Croner job；只有确实成功注册了当前 revision job 的 Loop 才进入恢复集合。这样恢复本身较慢时，正常 timer 已开始覆盖恢复 cutoff 之后的 occurrence。
...
6. 每次 catch-up enqueue 必须 await，并与在线 callback 使用同一个 in-flight work 集合；`stopAndDrain()` 停止 job 后等待两类工作，确保不会在 DB 关闭后继续写入。
...
在 `packages/server/src/store/runs.ts` 的 scheduled 分支中，复用/抽取 RFC 3339 解析与 canonical ISO 校验：
...
为测试增加仅内部可见的 composition override，使 `bootstrapServer()` 或等价测试构造可注入 `Clock` 与 `CronFactory`。生产路径仍固定使用 `systemClock` 和 `productionCronFactory`，不改变公开 API、protocol、数据库 schema 或 migration。

测试使用临时文件数据目录、真实 `127.0.0.1:0` listener、同一数据目录上的多次 open/close、FakeClock、FakeCronFactory 和 daemon runtime 的 Fake Runner。通过真实 HTTP 创建 Loop、poll、claim 与 report；不依赖真实 Claude 或等待真实分钟级 timer。

## 三、测试编组

### R1–R12：重启 Catch-up

- **R1**：短停机遗漏一次 occurrence；重启产生一个 pending，watermark 指向该 occurrence。
...
- **E4**：daemon claim 前再次重启，不新增 Run，原 pending 与 watermark 保持。
...
- 在 ADR-007 追加最终 catch-up、故障隔离和接受边界；在 ADR-008 更新“Batch 3 尚未实现”的历史边界并交叉引用最终裁决。
